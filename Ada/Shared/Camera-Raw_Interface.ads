...
package Camera.Raw_Interface is
  ...
  function Init (Flags : C.unsigned := 0) return Context with
    Import        => True,
    Convention    => C,
  ...
  function Get_Iwidth (Ctx : Context) return C.int with
    Import        => True,
    Convention    => C,
    External_Name => "libraw_get_iwidth";

  -- int libraw_get_iheight(libraw_data_t *lr);
  function Get_Iheight (Ctx : Context) return C.int with
    Import        => True,
    Convention    => C,
  ...
  function Str_Error (Code : C.int) return System.Address with
    Import        => True,
    Convention    => C,
