...
package body Persistent is
  ...
  Read_From_Storage : Boolean := False;


  function Storage_Is_Empty return Boolean is
  begin
    return not Read_From_Storage;
  ...
  begin
    ...
    begin
      The_Data.Storage := Kind'input(The_Stream);
      IO.Close (The_File);
  ...
  exception
  when Item: others =>
    Log.Write ("Persistent.Initialize", Item);
  end Initialize;
  ...
  exception
  when Item: others =>
    Log.Write ("Persistent.Finalize", Item);
  end Finalize;
