  ...
  procedure Show (Information : Telescope.Data) is
    ...
    begin
      if Time.Is_Defined (Information.Clock_Time) then
        ...
        if Information.Clock_Set then
          if Information.Clock_Synchronized then
            Gui.Set_Text (Clock_State, "synchronized");
          else
            Gui.Set_Text (Clock_State, "inaccurate");
          end if;
          if Information.Clock_Set_From_Pc then
      ...
      if Information.Mount_Connected then
        if Information.Mount_Synchronized then
    ...
    procedure Create_Interface is
      ...
      begin
        ...
        Temperature := Gui.Create (Display_Page, "Temperature", "",
                                   ...
                                   The_Size       => Text_Size,
                                   The_Title_Size => Title_Size);
        Server_Time := Gui.Create (Display_Page, "Server Time", "",
                                   Is_Modifiable  => False,
                                   The_Size       => Text_Size,
                                   The_Title_Size => Title_Size);
